...
| ✅ ERP integration: ERPNext order-level, live in CI; B2MML-flavoured file; REST — no Job Card confirmations, and no SAP, Odoo, Oracle or NetSuite connector | ✅ Analytics & shift reports | 🟡 Agent operations layer (MCP) — 85 tools and the write discipline; the run that puts a real model against the live surface is a command, not a gate |

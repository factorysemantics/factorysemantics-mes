...
## M7 — Speak ERP: connectors hardened ✅ *(the typed per-operation contract and its outbox landed 2026-09-03; the round trip against a real ERPNext v15.120.0 landed 2026-09-10 and runs in CI)*
...
**Where it stands (2026-09-14): both clauses are met.** The `ERPNext (live)` job stands up ERPNext v15.120.0 in containers pinned by digest, seeds the plant, and asserts the whole round trip against ERPNext's own documents — the custom fields, `produced_qty`, one submitted Manufacture stock entry, the comment — and a retried confirmation is asserted to leave exactly one stock entry (#17). The file adapter's symmetry tests pass and the contract is now published as a JSON Schema with worked example files and `fsmes erp validate` (#23). The port carries `requirements()`, `setup()` and `check()` with a conformance suite of eight obligations that ships in the package (#18), and [decision 0020](docs/decisions/0020-what-supported-means-for-an-erp-connector.md) says what *supported* requires. **Still open, and not part of the *done when*:** Job Card confirmations — whether Job Card is the right place at all is a question put to the ERPNext community on 2026-09-10 and not yet answered — and Odoo, SAP, Oracle and NetSuite, for which the contract and the suite exist and the connectors do not.

## M8 — As big or as small as the customer requires: packs + fleet ✅ *(all four pieces landed 2026-09-13 to 2026-09-14)*
...
**Where it stands (2026-09-14): done.** All four pieces of the design are built. A plant says its own name, keeps its own clock and states its profile (#34); `core_purity` and `no_tenant_literals` are in the suite and a `MES_MODULES` registry lets fourteen of the twenty-three modules be switched off, where off means the routes answer 404 and the screens are gone (#33); a plant is a **pack** — `plant.toml`, `fsmes pack check|apply|status|migrate`, master data as data, and the registry as a list of packs (#35, [the page](docs/operate/packs.md)); `fsmes fleet` manages the plants this installation owns and refuses every other one (#36); and `fsmes fleet console` serves one page that observes and has no write path, held there by four tests that parse its own source (#37). **Both clauses of the *done when* are met:** three lab packs with different modules enabled run from one wheel — one of them invented to disagree with the pack format, serving two modules fewer, proven by 404s and a shorter agent tool list — and the console shows them. Decision [0023](docs/decisions/0023-the-fleet-console-observes.md) was revised first (#32): the safety property is not read-only, it is *cannot touch a plant it does not own*.

**Not in M8 and not done:** site scoping on kernel tables. One database per plant is [decision 0021](docs/decisions/0021-one-database-per-plant.md); a `site` column on shared tables was not built and is not needed while that holds.
...
**As of 2026-09-14.** M0 bootstrapped 2026-08-28. M1–M6 landed 2026-08-31 by adopting MES-TWIN wholesale as the engine and building the missing modules on it (the decisions are recorded in `docs/decisions/`; the private-era history is summarised in `docs/history/private-era.md`). The repository went public on the evening of 2026-09-08, and `factorysemantics-mes` 0.1.0, 0.1.1 and 0.1.2 reached PyPI the same evening, with the container image on GHCR and the documentation at [docs.factorysemantics.com](https://docs.factorysemantics.com). CI runs lint and the test suite on Ubuntu and Windows across Python 3.12 and 3.13, on every push to `main` and every pull request, and `main` is green. Simulated plants run from a registry (`fsmes plant`), and the repository ships a templated systemd unit so a plant can run as a service; `fsmes score` measures the MES against the simulator's scripted truth on every change; the product exposes 85 agent tools and the simulator 13.

**0.2.0 is cut from `main` and tagged *on tagging* — the maintainer tags it and the steward fills this date in.** It carries thirty pull requests since 0.1.2 (2026-09-08): the ERPNext connector proven against a real ERPNext v15.120.0 in CI and its lessons promoted into the port with a conformance suite; shadow mode, inbound events by file, read-only SQL and MQTT, the published confirmation schema and validator, the shadow scorecard, backup and restore, and the engineers' front-door page; migrations that ship in the wheel, so a PyPI install can upgrade its own database, and the whole suite on a real PostgreSQL 16.15 in CI. See [the release page](docs/releases/0.2.0.md).

**M8 is done, 2026-09-14.** All four pieces of [the design](docs/design/m8-packs-and-fleet.md) landed: a plant that knows its own name, profile and clock (#34); the kernel/module wall — `core_purity`, `no_tenant_literals` and a `MES_MODULES` registry where fourteen of twenty-three modules can be switched off (#33); a plant as a **pack**, `plant.toml` with `fsmes pack check|apply|status|migrate` and the registry as a list of packs (#35); and the fleet — `fsmes fleet` managing only the plants this installation owns (#36) and a console that observes and has no write path (#37), under [decision 0023](docs/decisions/0023-the-fleet-console-observes.md) as revised (#32). Both clauses of the milestone's *done when* are met: three lab packs with different modules enabled run from one wheel, and the console shows them.
...
2. **Agent-first, mechanised** — **mostly done.** One `fsmes` MCP server with a tool file per area (`src/fsmes/mcp/`, [decision 0003](docs/decisions/0003-one-mcp-server.md)) carries all 85 tools. `tests/test_every_write_route_has_a_tool_or_a_reason.py` fails when an HTTP write route has neither a tool nor a stated reason, and `fsmes assist coverage` prints the whole table per role. Every route a screen calls now has a tool or a rule: the three that were excluded as plan phases (operation start, operation complete, document revise) became tools on 2026-09-26, and what remains excluded is excluded permanently — the approvals, which are a person's signature, a password, and signing somebody out. The `agent` role is not admin (`src/fsmes/services/capabilities.py`), `on_behalf_of` reaches every audit record, and idempotency keys are enforced in middleware. Open: the agent evals exist and their scoring runs in CI (`tests/test_agent_eval.py`), but the run that puts a real model against the live tool surface is a command a person types (`fsmes agent-eval`), not a gate.

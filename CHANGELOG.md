...
- `database is locked` under concurrent writes on SQLite. WAL and
  ...
  of plant readings to it. Writing transactions on SQLite now begin with
  ...
  it, so a new connection meeting a lock waits rather than failing.
  PostgreSQL is untouched — the change is gated on the SQLite dialect.

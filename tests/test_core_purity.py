...
DEFERRED: dict[tuple[str, str], str] = {
    ("fsmes.services.vocabulary", "fsmes.pack.check"):
        "the protected-word list, which is built by importing the product rather "
        "than typed anywhere: a word a plant authors may not spell a capability, "
        "a state, a KPI or a role. It lives with the pack checker because that "
        "is what needed it first, and the vocabulary lifecycle reads the same "
